...
class FAQCategoryForm(I18nModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs['locales'] = self.event.settings.get('locales') if PRTX == 'pretix' else self.event.locales
        super().__init__(*args, **kwargs)

    def save(self, *args, **kwargs):
        self.instance.event = self.event
        if not self.instance.position:
            self.instance.position = getattr(self.event.faq_categories.last(), 'position', 1)
...
class FAQForm(I18nModelForm):
    ...
    def save(self, *args, **kwargs):
        if not self.instance.position:
            self.instance.position = getattr(FAQ.objects.filter(category__event=self.event).last(), 'position', 1)
        super().save(*args, **kwargs)
